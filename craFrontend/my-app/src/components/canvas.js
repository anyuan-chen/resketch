...
export default function Canvas({ client, thing, roundNumber }) {
  ...
  const roundList = roundData.map((item) => {
    if (item.state === "correct") {
      return (
        <div className="flex justify-center items-center w-1/3 bg-[#0E312B] rounded-xl text-white py-3.5">
          {item.roundNumber}
        </div>
      );
    }
    if (item.state === "incorrect") {
      return (
        <div className="flex justify-center items-center w-1/3 bg-[#9D9D9D] rounded-xl text-white py-3.5">
          {item.roundNumber}
        </div>
      );
    }
    if (item.state === "not answered") {
      return (
        <div className="flex justify-center items-center w-1/3 bg-white rounded-xl text-black py-3.5 box-border">
          {item.roundNumber}
        </div>
      );
    }
  ...
  const selectPencilDiv = (
    <div className="h-1/2 flex bg-[#0E312B] justify-center items-center px-4 hover:cursor-pointer rounded-t-[1.45rem] select-none">
      <img
        src="../assets/Ellipse_inverted.svg"
        alt="Select Pencil"
        draggable="false"
      />
    </div>
  ...
  const selectEraserDiv = (
    <div className="h-1/2 flex bg-[#0E312B] justify-center items-center px-4 hover:cursor-pointer rounded-b-[1.45rem] select-none">
      <img
        src="../assets/Eraser_inverted.png"
        alt="Select Eraser"
        draggable="false"
      />
    </div>
      ...
      <div id="over-container">
        <div className="flex flex-col justify-center items-center w-2/3 space-y-8">
          ...
          <div className="flex w-full space-x-32 font-[Inter] font-bold">
            <div className="flex w-full space-x-8">{roundList}</div>
            <button
              onClick={() => {
                client.send(
                  JSON.stringify({
